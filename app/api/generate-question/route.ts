...
export async function POST(req: Request) {
  ...
  if (!apiKey) {
    ...
    console.log("DeepSeek API key not found, using mock question generator");
    const mockQuestion = generateMockQuestion(currentChinese, currentEnglish, topic || "this topic");
...
- Is realistic and natural`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert IELTS Writing tutor. Always respond with valid JSON only, no additional text.",
        },
        {
          role: "user",
          content: prompt,
    ...
    try {
      ...
      return NextResponse.json(generateMockQuestion(currentChinese, currentEnglish, topic || "this topic"));
    }
